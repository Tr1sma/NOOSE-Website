...
public class FraktionFotoStorageService : IFraktionFotoStorageService
{
    private readonly FileUploadOptions _options;
    private readonly string _basisPfad;

    public FraktionFotoStorageService(IWebHostEnvironment env, IOptions<FileUploadOptions> options)
    {
        _options = options.Value;
        _basisPfad = Path.IsPathRooted(_options.FraktionenPfad)
            ? _options.FraktionenPfad
            : Path.Combine(env.ContentRootPath, _options.FraktionenPfad);
    }
