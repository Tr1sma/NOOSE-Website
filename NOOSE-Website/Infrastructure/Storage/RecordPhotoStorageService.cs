...
/// <summary>Shared body of a record gallery store: files outside wwwroot, server-assigned names, bare-name reads only.</summary>
public abstract class RecordPhotoStorageService
{
    private readonly FileUploadOptions _options;
    private readonly string _basePath;

    protected RecordPhotoStorageService(IWebHostEnvironment env, FileUploadOptions options, string relativeOrRootedPath)
    {
        _options = options;
        _basePath = Path.IsPathRooted(relativeOrRootedPath)
            ? relativeOrRootedPath
            : Path.Combine(env.ContentRootPath, relativeOrRootedPath);
    }
