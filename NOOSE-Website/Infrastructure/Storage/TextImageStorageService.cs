...
/// <inheritdoc />
public class TextImageStorageService : ITextImageStorageService
{
    private readonly FileUploadOptions _options;
    private readonly string _basePath;

    public TextImageStorageService(IWebHostEnvironment env, IOptions<FileUploadOptions> options)
    {
        _options = options.Value;
        _basePath = Path.IsPathRooted(_options.TextImagesPath)
            ? _options.TextImagesPath
            : Path.Combine(env.ContentRootPath, _options.TextImagesPath);
    }

    public long MaxBytes => _options.TextImageMaxBytes;

    /// <summary>Images only: a pasted clipboard bitmap is a picture, an office document is an attack surface.</summary>
    ...
    public async Task<string> SaveAsync(Stream content, string contentType, CancellationToken cancellationToken = default)
    {
        ...
        // server-assigned name; nothing the pasting agent controls reaches the file system
