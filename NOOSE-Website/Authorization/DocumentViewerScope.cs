...
public readonly record struct DocumentViewerScope(bool MayClassified, bool IsTru, bool IsHrb, bool IsLeadership, bool IsAdmin, string? MeId)
{
    ...
    public static DocumentViewerScope From(ClaimsPrincipal user)
        => new(user.MayClassifiedRead(), user.IsTRU(), user.IsHRB(), user.IsLeadership(), user.IsAdmin(), user.GetAgentId());
