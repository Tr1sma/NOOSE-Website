...
public class NooseiRecordTypesTests
{
    ...
    public void German_NeverFallsThroughToTheClrName()
    {
        ...
        Assert.Equal("Eintrag", NooseiRecordTypes.German("WatchlistEntry"));
    }
    ...
    [Theory]
    [InlineData("Job")]
    [InlineData("EvidenceItem")]
    [InlineData("KassenBuchung")]
    ...
    public void Clr_WithACapability_RefusesATypeTheToolCannotHandle()
    {
        ...
        // so a job reaching lies_akte or an anchor would answer past its own visibility helper
        Assert.Null(NooseiRecordTypes.Clr("Aufgabe", NooseiUse.Read));
        Assert.Equal("Job", NooseiRecordTypes.Clr("Aufgabe", NooseiUse.Search));
    ...
    {
        var builder = NamedTypes(Source("Services", "Llm", "DossierContextBuilder.cs"));
