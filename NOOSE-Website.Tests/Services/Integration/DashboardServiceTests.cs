...
public sealed class DashboardServiceTests
{
    ...
    [Fact]
    public async Task GetLastChangesAsync_ResolvesRecords_NewestFirst()
    {
        using var ctx = new SqliteTestContext();
        var (svc, _, _) = Build(ctx);
        ...
        using (var db = ctx.NewContext())
        {
            db.People.Add(Per("p1", "Alice"));
            db.Factions.Add(Fac("f1", "Gang"));
            db.AuditLogs.Add(new AuditLog { EntityType = nameof(Person), EntityId = "p1", Action = AuditAction.Created, Timestamp = t0, AgentName = "Bob" });
            db.AuditLogs.Add(new AuditLog { EntityType = nameof(Faction), EntityId = "f1", Action = AuditAction.Modified, Timestamp = t0.AddHours(1), AgentName = "Bob" });
            db.SaveChanges();
        }

        var list = await svc.GetLastChangesAsync(isLeadership: true, meId: "lead");

        Assert.Equal(2, list.Count);
        Assert.Equal(DashboardRecordType.Faction, list[0].RecordType);   // newer first
        Assert.Equal("Gang", list[0].RecordName);
        Assert.Equal(AuditAction.Modified, list[0].Action);
        Assert.Equal(DashboardRecordType.Person, list[1].RecordType);
        Assert.Equal("Alice", list[1].RecordName);
    }

    [Fact]
    public async Task GetLastChangesAsync_RollsUpChildToParent()
    {
        using var ctx = new SqliteTestContext();
        var (svc, _, _) = Build(ctx);
        using (var db = ctx.NewContext())
        {
            db.People.Add(Per("p1", "Alice"));
            db.PersonDocs.Add(new PersonDoc { Id = "doc1", PersonId = "p1", Outcome = MeasureOutcome.RunningStill, Timestamp = DateTime.UtcNow });
            db.AuditLogs.Add(new AuditLog { EntityType = nameof(PersonDoc), EntityId = "doc1", Action = AuditAction.Modified, Timestamp = DateTime.UtcNow, AgentName = "Bob" });
            db.SaveChanges();
        }
        ...
        Assert.Single(list);
        Assert.Equal(DashboardRecordType.Person, list[0].RecordType);   // doc rolled up to its person
        Assert.Equal("p1", list[0].RecordId);
        Assert.Equal("Alice", list[0].RecordName);
        Assert.Equal("Dok", list[0].Detail);
    }

    [Fact]
    public async Task GetLastChangesAsync_NonLeadership_ExcludesClassified()
    {
        using var ctx = new SqliteTestContext();
        var (svc, _, _) = Build(ctx);
        var t0 = new DateTime(2026, 5, 1, 0, 0, 0, DateTimeKind.Utc);
        using (var db = ctx.NewContext())
        {
            db.People.Add(Per("p1", "Public"));
            db.People.Add(Per("p2", "Secret", p => p.IsClassified = true));
            db.AuditLogs.Add(new AuditLog { EntityType = nameof(Person), EntityId = "p1", Action = AuditAction.Modified, Timestamp = t0, AgentName = "Bob" });
            db.AuditLogs.Add(new AuditLog { EntityType = nameof(Person), EntityId = "p2", Action = AuditAction.Modified, Timestamp = t0.AddHours(1), AgentName = "Bob" });
            db.SaveChanges();
        }

        var list = await svc.GetLastChangesAsync(isLeadership: false, meId: "me");

        Assert.Single(list);
        Assert.Equal("Public", list[0].RecordName);
    }

    [Fact]
    public async Task GetLastChangesAsync_Taskforce_MembershipGates()
    {
        using var ctx = new SqliteTestContext();
        var (svc, _, _) = Build(ctx);
        var t0 = new DateTime(2026, 5, 1, 0, 0, 0, DateTimeKind.Utc);
        using (var db = ctx.NewContext())
        {
            ...
            db.Taskforces.Add(Tf("t1", "Mine"));
            db.Taskforces.Add(Tf("t2", "Theirs"));
            db.TaskforceAgents.Add(new TaskforceAgent { TaskforceId = "t1", AgentId = "me" });
            db.AuditLogs.Add(new AuditLog { EntityType = nameof(Taskforce), EntityId = "t1", Action = AuditAction.Modified, Timestamp = t0, AgentName = "Bob" });
            db.AuditLogs.Add(new AuditLog { EntityType = nameof(Taskforce), EntityId = "t2", Action = AuditAction.Modified, Timestamp = t0.AddHours(1), AgentName = "Bob" });
            db.SaveChanges();
        }

        var list = await svc.GetLastChangesAsync(isLeadership: false, meId: "me");

        // only the taskforce the caller belongs to is in the feed.
        Assert.Single(list);
        Assert.Equal(DashboardRecordType.Taskforce, list[0].RecordType);
        Assert.Equal("t1", list[0].RecordId);
    }

    [Fact]
    public async Task GetLastChangesAsync_RespectsMax()
    {
        using var ctx = new SqliteTestContext();
        var (svc, _, _) = Build(ctx);
        var t0 = new DateTime(2026, 5, 1, 0, 0, 0, DateTimeKind.Utc);
        using (var db = ctx.NewContext())
        {
            for (var i = 0; i < 5; i++)
            {
                db.People.Add(Per($"p{i}", $"Person{i}"));
                db.AuditLogs.Add(new AuditLog { EntityType = nameof(Person), EntityId = $"p{i}", Action = AuditAction.Modified, Timestamp = t0.AddMinutes(i), AgentName = "Bob" });
            }
            db.SaveChanges();
        }

        var list = await svc.GetLastChangesAsync(isLeadership: true, meId: "lead", max: 2);

        Assert.Equal(2, list.Count);
    }

    [Fact]
    public async Task GetLastChangesAsync_DeletedRecord_MarkedDeleted()
    {
        using var ctx = new SqliteTestContext();
        var (svc, _, _) = Build(ctx);
        using (var db = ctx.NewContext())
        {
            db.People.Add(Per("p1", "Ghost", p => { p.IsDeleted = true; p.DeletedAt = DateTime.UtcNow; }));
            db.AuditLogs.Add(new AuditLog { EntityType = nameof(Person), EntityId = "p1", Action = AuditAction.Deleted, Timestamp = DateTime.UtcNow, AgentName = "Bob" });
            db.SaveChanges();
        }

        var list = await svc.GetLastChangesAsync(isLeadership: true, meId: "lead");

        // trashed record is still resolvable via IgnoreQueryFilters and flagged as deleted.
        Assert.Single(list);
        Assert.Equal("Ghost", list[0].RecordName);
        Assert.True(list[0].RecordDeleted);
    }

    [Fact]
    public async Task GetLastChangesAsync_EmptyAudit_ReturnsEmpty()
    {
        using var ctx = new SqliteTestContext();
        var (svc, _, _) = Build(ctx);

        Assert.Empty(await svc.GetLastChangesAsync(isLeadership: true, meId: "lead"));
    }
}
